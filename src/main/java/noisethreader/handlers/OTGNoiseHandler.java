...
public abstract class OTGNoiseHandler {
	
	private static double[] finalNoiseArray;
	
	public static double[] generateOTGNoiseOctaves(NoiseGeneratorPerlinOctaves noiseGeneratorPerlinOctaves, double[] noiseArray, int xOffset, int yOffset, int zOffset, int xSize, int ySize, int zSize, double xScale, double yScale, double zScale, int octaves, int octaveSplitSize, int octaveSplitAmount) {
		finalNoiseArray = noiseArray;
		
		try {
			IntStream.range(0, octaveSplitAmount).parallel().forEach(split -> OTGNoiseHandler.combineArrays(((INoiseGeneratorPerlinOctaves)noiseGeneratorPerlinOctaves).noisethreader$generateNoiseOctavesThreaded(null, xOffset, yOffset, zOffset, xSize, ySize, zSize, xScale, yScale, zScale, split * octaveSplitSize, Math.min(octaves, (split + 1) * octaveSplitSize)), xSize, ySize, zSize));
		}
		catch(Exception ex) {
